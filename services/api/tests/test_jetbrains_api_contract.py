"""Guard: every endpoint the JetBrains plugin calls must still exist in this
API. Same idea as test_cli_api_contract.py / test_vscode_api_contract.py —
`services/jetbrains/api_contract.json` names every path the plugin depends
on; if a router moves or renames one, this test names the plugin feature
that just broke."""
import json
...
CONTRACT = Path(__file__).resolve().parents[2] / "jetbrains" / "api_contract.json"
PREFIX = "/api/v1"
...
def contract() -> list[dict]:
    assert CONTRACT.exists(), f"JetBrains contract missing at {CONTRACT}"
